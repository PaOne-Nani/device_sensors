...
@objc class AppDelegate: FlutterAppDelegate {
    ...
    private func handleSensorMethodCall(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "toggleAccelerometer":
            ...
            toggleAccelerometer(enabled: enabled, result: result)
        case "toggleGyroscope":
            let arguments = call.arguments as? [String: Any]
            let enabled = arguments?["enabled"] as? Bool ?? false
            toggleGyroscope(enabled: enabled, result: result)
        default:
            result(FlutterMethodNotImplemented)
        }
    }
    ...
    // MARK: - Sensor Toggle Methods
    
    private func toggleAccelerometer(enabled: Bool, result: @escaping FlutterResult) {
        isAccelerometerEnabled = enabled
        if enabled && motionManager.isAccelerometerAvailable {
    ...
    private func toggleGyroscope(enabled: Bool, result: @escaping FlutterResult) {
        isGyroscopeEnabled = enabled
        if enabled && motionManager.isGyroAvailable {
    ...
    func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        ...
        if isAccelerometerEnabled {
            motionManager.startAccelerometerUpdates(to: .main) { [weak self] (data, error) in
                if let error = error {
                    self?.sensorEventSink?(FlutterError(code: "ACCEL_ERROR", message: error.localizedDescription, details: nil))
                    return
                }
                if let data = data {
                    let accelDict: [String: Any] = [
                        "sensorType": "accelerometer",
                        "x": data.acceleration.x,
                        "y": data.acceleration.y,
                        "z": data.acceleration.z
                    ]
                    self?.sensorEventSink?(accelDict)
                }
            }
        }
